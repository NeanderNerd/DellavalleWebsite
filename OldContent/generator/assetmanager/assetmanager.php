...
Function recursive($sPath,$sPath_base,$sName)
  ...
  global $currFolder;
	...
	{
		if(getExt($sName) != 'php' && strstr($sName,"_vti")==false) {
			if($currFolder==$sPath)
  ...
  while($sItem=readdir($oItem))
    {
    if($sItem=="."||$sItem==".."||getExt($sName) == 'php')
      continue;
    else
      {
...
function writeFileSelections()
  ...
  for ($i=0; $i<count($aItem); $i++)
	...
	if(strstr($sItem, '_vti') != false)
		continue;
	if(strstr($sItem, 'backup_') != false)
		continue;

    if($sItem=="."||$sItem==".."||$sItem=='db.php')
      continue;
    else
      {
